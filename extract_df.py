...
def get_gate_values(directory="./raw_data", remove_path="./data/removed.txt"):
    ...
    total_df = pd.DataFrame(index=range(0,20000), columns=["Name", "Type", "Group", "Trace"])
    index_counter = 0
                ...
                if "inf.dat" in file_name:
                    ...
                    if (np.amax(data)>1) or (np.amin(data)<0):
                        ...
                        if not np.isnan(data).any():
                            ...
                            total_df.loc[index_counter]["Variable"] = "INF"
                            total_df.loc[index_counter]["Group"] = group
                            total_df.loc[index_counter]["Trace"] = pd.Series([data])

                            index_counter += 1

    # save "pure" version without taus
    keep_save = total_df.dropna()
    print(len(keep_save))
    keep_save.to_pickle("data/extracted_df.pickle")

    # add taus and put them all into one big datafarme
    for folder_name in folder_list:
        # get tuple with:
        #   tuple[0] = folder name
        #   tuple[2] = list of file
            ...
            for file_name in file_tuple[2]:
                ...
                if "tau.dat" in file_name:
                    ...
                    if not np.isnan(data).any():
                        ...
                        total_df.loc[index_counter]["Name"] = name
                        total_df.loc[index_counter]["Type"] = type_ion
                        total_df.loc[index_counter]["Variable"] = "TAU"
                        total_df.loc[index_counter]["Group"] = "UNKOWN"
                        total_df.loc[index_counter]["Trace"] = pd.Series([data])
    ...
    total_df.to_pickle("data/complete_df.pickle")
    print(len(total_df))
    return keep_save
